...
class BiLSTMpool(nn.Module):
    ...
    def __init__(self, output_size, embeddings, trainable_emb=False):
        ...
        self.linear = nn.Linear(2 * 3 * self.hidden_size, output_size)
    ...
    def forward(self, x, lengths):
        ...
        # Compute last output of BiLSTM for each sentence.
        last = torch.zeros(batch_size, 2*self.hidden_size).float().to(DEVICE)
        for i in range(batch_size):
            if lengths[i] > seq_len:
                last[i] = lstm_out[i, seq_len-1, :]
            else:
                last[i] = lstm_out[i, lengths[i]-1, :]
        # Compute mean of the outputs of the BiLSTM for each sentence.
        mean_pool = torch.zeros(batch_size, 2*self.hidden_size).float().to(DEVICE)
        for i in range(batch_size):
            if lengths[i] > seq_len:
        ...
        max_pool = torch.zeros(batch_size, 2*self.hidden_size).float().to(DEVICE)
        for i in range(batch_size):
            if lengths[i] > seq_len:
